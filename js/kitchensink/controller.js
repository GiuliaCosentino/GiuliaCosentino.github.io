...
function createPluses() {
	...
	$("#imageLoader").change(addcardfilechange);
...
function addcardimageloaded(){
	...
	fabric.Image.fromURL(url, function (image) {
		...
		image.set({
				...
				hasControls: false,
				selectable: true,
				scaleX: 111.6 / img.width,
				scaleY: 155.85 / img.height
			})
			.setCoords();
image._element.setAttribute("class","owncard");
		canvas.add(image);
	});
...
function createCards() {
	...
	for (key in cards) {
		...
		console.log(key, cards[key].min)

		for (var i = cards[key].min; i <= cards[key].max; i++) {
			var url = assetsfolder + key + '/cards' + i + '.png';
			el += '<button type="button" class="btn card" data-index="' + i + '" data-type="' + key + '" style="background-image: url(' + url + ')"></button>';
		}
	...
	$("ul.nav li a").hover(function (e) {
		var key = $(this).attr("href").replace("#", "");
		if ($(this).parent().hasClass("active")) {
			$(this).css("background-color", cards[key].color);
...
function addAccessors($scope) {
	...
	$scope.addContrastFilter = function () {
		setActiveStyle('filters', [new fabric.Image.filters.Contrast({
			contrast: 0.7
		})]);
		var obj = canvas.getActiveObject();
		obj.applyFilters && obj.applyFilters();
